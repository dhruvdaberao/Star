...
const PostCard: React.FC<PostCardProps> = ({ post, currentUser, onLike, onComment, onDeletePost, onDeleteComment, onViewProfile }) => {
    ...
    <div className="bg-surface border border-border rounded-2xl shadow-md mb-6">
      ...
      <div className="flex justify-around items-center p-1 text-secondary font-semibold">
        ...
        >
         ...
         <ShareButton 
            shareData={{
                title: `Check out this post by ${post.author.name} on Tribe!`,
                text: post.content,
                url: `https://tribe.app/posts/${post.id}`,
            }} 
            className="flex items-center space-x-2 p-2 rounded-lg hover:bg-background w-full justify-center transition-colors"
        >
            <ShareIcon />
        ...
        <div className="p-4 border-t border-border">
          ...
          <form onSubmit={handleCommentSubmit} className="flex items-center space-x-2 mb-4">
            ...
            <input
              ...
              className="flex-1 bg-background border border-border rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-accent text-primary text-sm"
            />
            <button type="submit" className="text-accent font-semibold text-sm disabled:text-secondary" disabled={!commentText.trim()}>Post</button>
          </form>
