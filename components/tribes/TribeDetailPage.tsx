...
const TribeDetailPage: React.FC<TribeDetailPageProps> = ({ tribe, currentUser, onSendMessage, onBack, onViewProfile, onEditTribe }) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] md:h-[calc(100vh-5rem)] bg-surface rounded-2xl border border-border shadow-md overflow-hidden">
      {/* Header (Fixed) */}
      <div className="flex items-center p-3 border-b border-border flex-shrink-0">
        ...
        <div className="ml-auto flex items-center space-x-2">
            <ShareButton
                shareData={{
                    title: `Join the '${tribe.name}' Tribe!`,
                    text: tribe.description,
                    url: `https://tribe.app/tribes/${tribe.id}`,
                }}
                className="p-2 text-secondary hover:text-primary rounded-lg hover:bg-background"
            >
                <ShareIcon />
            </ShareButton>
